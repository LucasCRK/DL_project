...
class ProteinGraph(nx.Graph):
    ...
    def parse_pdb(self):
        ...
        #in case of missing densities
        p=atomic_df.groupby(['resi_num'])
        for symbol, group in p:
            if 'CA' in group['atom'].to_list():
                pass            
            else:
                atomic_df.drop(index=group.index.to_list(), inplace=True)
    ...
    def compute_node_features(self, node, debug=False):
        ...
        self.node[node]["features"] =feature_list 
